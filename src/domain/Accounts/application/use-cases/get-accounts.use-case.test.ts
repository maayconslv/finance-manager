...
describe("Application - Get accounts - Use case", () => {
  ...
  it("should be able to get all user accounts correctly", async () => {
    ...
    bankAccounts.forEach((item, index) => {
      ...
      expect(item.accountName).to.be.equal(bankAccountsData[index]!.accountName);
      expect(item.bankName).to.be.equal(bankAccountsData[index]!.bankName);
      expect(item.currentBalance).to.be.equal(bankAccountsData[index]!.currentBalance.toBRL());
      expect(item.initialBalance).to.be.equal(bankAccountsData[index]!.initialBalance.toBRL());
    });
    expect(wallet.id).to.be.equal(walletData.id);
    expect(wallet.currentBalance).to.be.equal(walletData.currentBalance.toBRL());
    expect(wallet.userId).to.be.equal(walletData.userId);
  });
